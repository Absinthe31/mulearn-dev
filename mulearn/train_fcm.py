...
def fit_model(params, X, y_t):

    scores = []

    for train, test in cv.split(X, y_t):
        ...
        score = sklearn.metrics.mean_squared_error(
            y_pred=np.array([y[i] for y in model.soft_predict(X_test)]),
...
for digit in [0]:
    ...
    res = Parallel(n_jobs=1, verbose=0)(delayed(fit_model)(params, X_train, y_train) for params in param_candidates)
    
    best_result = min(res, key=lambda x: x[1])
    
    print('   ', best_result)

    best_model = FCM(n_clusters=best_result[0]['n_clusters'], m=best_result[0]['m'])
    ...
    for pred, y in zip(best_model.soft_predict(X_test), y_test):
        matrix[y][np.argmax(pred)] += 1 
    ...
    logger.info(log_model(model=best_model, model_name='FCM', acc=score, dataset=f'MNIST_FULL_{digit}', serialize=True))
