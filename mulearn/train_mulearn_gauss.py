...
kernel_name_mapping = {
    ...
    'GaussianKernel(sigma=0.1)' : GaussianKernel(sigma=0.1),
    'GaussianKernel(sigma=0.01)' : GaussianKernel(sigma=0.01),
    'GaussianKernel(sigma=10)' : GaussianKernel(sigma=10),
    'GaussianKernel(sigma=100)' : GaussianKernel(sigma=100)
...
for digit in digits:
    ...
    #gaussian_kernels = [GaussianKernel(),  GaussianKernel(sigma=0.1), GaussianKernel(sigma=0.01),  GaussianKernel(sigma=10), GaussianKernel(sigma=100)]
    ...
    parameters_gaussian = {'c' : [x for x in np.logspace(-2,2,5)],
                           ...
                           'fuzzifier' : [ExponentialFuzzifier(profile='fixed'), ExponentialFuzzifier(profile='infer')] +
                                         ...
                                         [LinearFuzzifier(profile='infer'), LinearFuzzifier(profile='fixed')]}
    
    params = parameters_gaussian
    
    model = FuzzyInductor(solver=GurobiSolver(adjustment='auto'))
    
    cv = StratifiedKFold(n_splits=5)
    
    train = np.array([x for x in range(len(X_train))]).reshape(-1,1)
    ...
    for alpha in np.linspace(0.1,0.9,9):
        acc = accuracy_score(y_pred=best_model.predict(train, alpha=alpha), y_true=y_train)
        if acc > best_acc:
            best_alpha = alpha


    kernel_names =  sorted(os.listdir(path + '/GaussianKernels/'))
    
    precomuputed_kernels =  gaussian_kernels 

    for kernel_name, precomuputed_kernel in zip(kernel_names, precomuputed_kernels):
        if np.array_equal(best_model.k.kernel_computations, precomuputed_kernel.kernel_computations):
            print(kernel_name)
            break

    best_model.k = kernel_name_mapping[kernel_name]

    print(best_model)

    best_model._fix_object_state(X_train, y_train)

    print('evaluating model on test set')

    score = accuracy_score(y_pred=best_model.predict(X_test, alpha=best_alpha), y_true=y_test)
